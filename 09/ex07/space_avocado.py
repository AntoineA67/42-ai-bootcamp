...
def main():
	...
	for degree in range(1, 5):
		for lambda_ in np.arange(1, step=.2):

			# Generate random thetas and initialize linear regression class
			lr = MyRidge(np.random.rand(3 * degree + 1, 1), 0.01, 20000, lambda_=lambda_)
			...
			fig.text(x=0., y=.98, s=f'Degree: {degree}  Lambda: {lambda_}')
			...
			new_metric = pd.DataFrame({'degree': [degree], 'theta': [' '.join(str(x).strip('[]') for x in lr.theta)],'mse_train': [mse_train], 'mse_test': [mse_test], 'lambda': [lambda_]})
			metrics = pd.concat([metrics, new_metric], ignore_index=True, axis=0)

			# Plot 3d representation for best model:
			if degree == 3 and lambda_ == .0:
				# Create subplots
				fig3d = plt.figure()
